...
---
## 5. CODE AUTHENTICITY & HUMAN-LIKE STYLE
To ensure the codebase looks like it was written by a Senior Software Engineer (Varfolomiy) and not an LLM, strictly follow these stylistic rules:

* **Minimalist Commenting:** Avoid stating the obvious. Only comment on complex logic, hacks, or mathematical formulas (like ICM or reward scales). Use concise, technical language.
* **Avoid Generic Naming:** Do not use `data_list`, `temp_var`, or `process_results`. Use domain-specific terminology (e.g., `obs_buffer`, `tick_delta`, `reward_sum`, `entity_map`).
* **Incorporate "Human Cruft":** Occasionally include `TODO` notes with specific technical debt mentions (e.g., `# TODO: migrate to shared memory for lower latency`).
* **Shorthand & Pythonic Idioms:** Prefer list comprehensions, `f-strings`, and proper context managers (`with` statements) over verbose loops.
* **No "AI Politeness":** The code should be clinical and efficient. Do not include excessive docstrings for self-explanatory internal methods.
* **Directory Structure:** Adhere to the established project structure (src/, configs/, ui/). Ensure all imports are absolute (e.g., `from src.logger import log_anomaly`).
