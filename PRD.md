...
1. `python -m src.bridge_maker --help` shows the product CLI.
2. `python -m src.bridge_maker suggest --repo examples` writes suggestion files.
3. `python -m src.bridge_maker export --adapter examples/annotated_dummy.py --out runs/dummy` writes all contract files.
4. The exported `state_map.json` validates against `StateMap`.
5. `python -m src.bridge_maker smoke --adapter examples/annotated_dummy.py` runs a 20-step Gymnasium loop without CE, Ghidra, Ray, Wandb, Modal, or Azure.

---

## 6. Product Direction

The old CoQ/socket PRD is archived as `PRD_old_coq_socket.md`. The active direction is SDK/Adapter-first. The grey-box stack remains valuable, but it is now an assist path rather than the main promise.
